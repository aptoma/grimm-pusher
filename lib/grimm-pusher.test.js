...
describe('add()', () => {
	...
	describe('batch processing', () => {
		it('should default to a batch size of 1', () => {
			subject = createService(optionsWithoutMaxBatchSize);
		...
		it('should trigger process if max batch size is reached', () => {
			subject = createService(Object.assign({}, optionsWithoutThrottling, {maxBatchSize: 2}));
	...
	describe('throttling', () => {
		it('should not trigger processing before throttle delay has passed', () => {
			subject = createService(Object.assign({}, options, {throttleMs: 1000, maxBatchSize: 2}));
		...
		it('should trigger processing when throttle delay has passed', (done) => {
			subject = createService(Object.assign({}, options, {throttleMs: 5, maxBatchSize: 2}));
		...
		it('should trigger processing when throttling is set to 0 ms', () => {
			subject = createService(Object.assign({}, options, {throttleMs: 0, maxBatchSize: 2}));
		...
		it('should trigger processing when max batch size is reached, even when throttling is enabled', () => {
			subject = createService(Object.assign({}, options, {throttleMs: 100, maxBatchSize: 1}));
			mockGrimmTransportSendOK();
			subject.add({name: 'foo'});
...
describe('process()', () => {
	...
	it('should emit success event when there are no pending events', (done) => {
		const subject = createService(options);
	...
	it('should emit error event when apikey is missing', (done) => {
		const subject = createService(Object.assign({}, options, {apikey: null}));
